...
const FIXES = {
	'seven-of-wands'(c) {
		c.correspondences.decan = c.correspondences.decan.replace(
			'first decan of Leo',
			'third decan of Leo'
		);
	},
	...
	'king-of-cups'(c) {
		...
		if (s)
			s.note = s.note
				.replace(' and the Grail tradition.', '.')
				.replace(', love, relationships.', ', love, and relationships.');
	},
	'seven-of-swords'(c) {
		...
		if (s) {
			...
			s.note =
				"Joan Bunning's practical reading of the Seven of Swords — its themes of running away, hidden action, lone-wolf strategy and trying to get away with something — with upright and reversed guidance.";
		}
	},
	'eight-of-swords'(c) {
...
function cleanCorr(corr) {
	...
	if (h && !((h.letter || '').trim() || (h.name || '').trim() || (h.meaning || '').trim()))
		delete corr.hebrewLetter;
	return corr;
}
...
// ---- audit ----
const over = cards.filter(
	(c) => wc(c.symbolismProse) > 180 || wc(c.upright) > 130 || wc(c.reversed) > 130
);
console.log(`Wrote ${cards.length} cards -> src/lib/data/cards.json`);
console.log(`Over-cap after fixes: ${over.length}`);
over.forEach((c) =>
	console.log(
		`  ${c.id}: prose ${wc(c.symbolismProse)}, up ${wc(c.upright)}, rev ${wc(c.reversed)}`
	)
);
...
for (const id of Object.keys(FIXES)) {
	...
	console.log(
		`  ${id}: prose ${wc(c.symbolismProse)}/180, up ${wc(c.upright)}/130, rev ${wc(c.reversed)}/130, decan="${c.correspondences.decan ?? ''}"`
	);
}
